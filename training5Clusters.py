...
for z in range(X_train.shape[2]):
    ...
    ax[z, 3].set_yticklabels([])
    ax[z, 4].set_yticklabels([])

    ax[z, 2].set_xlabel("Simulation Time (second)")

    for j in range(C):
        ax[z, j].set_ylim((np.min(X_train[:,:,z]), np.max(X_train[:,:,z])))
        line2,=ax[z, j].plot(T[-1,:], bars[j][:,z], color='r', label='DTW BA', linewidth=.8)
        ax[0, j].set_title(f"Cluster {j+1}")
        #ax[z, j].set_title(f"Klaster {j+1}")
        if j==C-1:
